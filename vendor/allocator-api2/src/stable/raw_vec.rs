...
impl<T, A: Allocator> RawVec<T, A> {
    ...
    pub fn reserve(&mut self, len: usize, additional: usize) {
        ...
        // pgrust: out of line, as the comment above intends and std does (upstream 0.2.21 had
        // #[inline(always)] here, which put the whole growth path at every reserve site).
        #[cold]
        #[inline(never)]
    ...
    #[cfg(not(no_global_oom_handling))]
    // pgrust: out of line (std's RawVec::grow_one is #[inline(never)]); upstream 0.2.21 inlined
    // grow_amortized + finish_grow + the allocator's allocate/grow into every Vec::push site.
    #[inline(never)]
...
impl<T, A: Allocator> RawVec<T, A> {
    ...
    // pgrust: no #[inline(always)]; LLVM inlines it into the out-of-line growth entry points.
    ...
    // it's less critical.
    // pgrust: no #[inline(always)], as grow_amortized.
...
// pgrust: #[inline(never)], as std's finish_grow: one copy per allocator type.
#[inline(never)]
fn finish_grow<A>(
